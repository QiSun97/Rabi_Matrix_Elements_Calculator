...
disp('Shifts')
disp('S_L1 [Gamma_APi]')
disp(vpa(Shift_L1/Gamma_APi,5))
disp('S_L2 [Gamma_BSigma]')
disp(vpa(Shift_L2/Gamma_BS,5))
...
% figure
%%
VelocitiesL=-150:1:-121;
VelocitiesR=121:1:150;
Velocities=[VelocitiesL,VelocitiesR];
%%
% u=0;
Forces_array_extended_more=zeros(60,1);
parfor vel_index=1:60
...
%%
save('BaH_pcf_molasses.mat','Final_force');

%%
figure
plot(Velocities,Forces_array)
ylim([-35,35])
%%
...
x=simplify(x)
%%
current_unit=(k_api*Gamma_APi/2)
new_unit=(1.98*10^(-21)/hbar)*10^(-18)
change_factor=current_unit/new_unit
%%
Mid=load('BaH_pcf_molasses_full.mat');
Mid=Mid.Forces_array;
Outer=load('BaH_pcf_molasses_full_extended.mat');
Outer=Outer.Forces_array_extended;
Far=load('BaH_pcf_molasses_full_extended_more.mat');
Far=Far.Forces_array_extended_more;
%%
Final_force=[Far(1:30)',Outer(1:60)',Mid',Outer(61:120)',Far(31:end)']*change_factor;
%%
figure
plot(-150:1:150,Final_force)
